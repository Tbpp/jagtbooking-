...
fane_book, fane_tjek_dato, fane_fuld_oversigt = st.tabs([
    "🆕 Opret Booking", "🔍 Tjek Specifik Dato", "📅 Den Fulde Kalenderoversigt & Aflysning"
])
...
with fane_book:
    ...
    valgt_jaeger_id = st.selectbox(
        "Vælg dit navn på listen:", options=list(st.session_state.jaegere.keys()),
    ...
    valgt_dato = st.date_input("Vælg dato for jagten (Maks 14 dage frem):", min_value=idag, max_value=fjorten_dage_frem, key="dato_valg")
        ...
        if valider_id_input.strip() != str(valgt_jaeger_id):
            ...
            if booking_noegle in st.session_state.bookinger:
                ...
                st.session_state.bookinger[booking_noegle] = {
                    "jaeger_id": valgt_jaeger_id, "navn": st.session_state.jaegere[valgt_jaeger_id],
                    "tidspunkt": valgt_tidspunkt, "notat": notat_input if notat_input.strip() else "-"
                }
                st.success(f"✅ Godkendt! {st.session_state.jaegere[valgt_jaeger_id]} har booket {st.session_state.omraader[valgt_omraade_id]}.")
...
with fane_tjek_dato:
    ...
    for omr_id, omr_navn in st.session_state.omraader.items():
        ...
        if morgen_noegle in st.session_state.bookinger:
            ...
            st.info(f"🌅 **Morgen**: {omr_navn} er optaget af Nr. {b['jaeger_id']} - {b['navn']}")
            fundet_booking = True
        else:
        ...
        if aften_noegle in st.session_state.bookinger:
            ...
            st.warning(f"🌇 **Aften**: {omr_navn} er optaget af Nr. {b['jaeger_id']} - {b['navn']}")
            fundet_booking = True
        else:
...
# --- FANE 3: KALENDEROVERSIGT & AFLYSNING ---
with fane_fuld_oversigt:
    ...
    if not st.session_state.bookinger:
        ...
        for noegle, info in list(st.session_state.bookinger.items()):
            ...
            omraade_navn = st.session_state.omraader[omraade_id_int] if omraade_id_int in st.session_state.omraader else f"ID: {omraade_id_int}"
            tabel_data.append({
                "Nøgle": noegle, "Dato": dato_del, "Tidspunkt": tid_del,
                "Jagtområde": omraade_navn, "Jæger": f"Nr. {info['jaeger_id']} - {info['navn']}", "Notat": info["notat"]
            })
        ...
        if tabel_data:
            ...
            slet_valg = st.selectbox("Vælg den booking du vil fjerne:", options=tabel_data, format_func=lambda x: f"{x['Dato']} - {x['Jagtområde']}")
            ...
            if st.button("Slet valgte booking", type="secondary"):
                if slet_id_validering.strip() == str(st.session_state.bookinger[slet_valg["Nøgle"]]["jaeger_id"]):
                    del st.session_state.bookinger[slet_valg["Nøgle"]]
                    st.success("📌 Bookingen er blevet slettet!")
                    st.rerun()
                else:
                    st.error("❌ Forkert medlemsnummer.")
